...
public class TC0020PortNumberApprove extends BaseClass 
{
	...
	{
		BaseClass.stepComplete("------------------ Starting port number action with approve. -----------------------", "");
		...
		//DebugTimeout(3, "Freeze");
		
		CommonTestSteps.Logout();
		
		BaseClass.stepComplete("Run port number action complete. Now will approve the order.", "");
		...
		PortNumber.VerifyOrderDetailsPagePostApproval(ApproverAction.approve);
		
		// the order details page is open. it has synced with command so now the history section can be verified. 
		PortNumber.VerifyOrderDetailsHistoryAccessoriesAfterApproval(ApproverAction.approve);
